        ...
        for block in range(nblock):
            for record in whichdata:
                ...
                #                 approx_grad = True) # , disp=0, iprint=-1)
                    ...
                    method == "openopt"
                ):  # use OpenOpt's routines - usually slower, but sometimes they converge better
                    if not HAVE_OPENOPT:
                        raise Exception(
                            "Requested openopt fitting method but openopt is not installed."
                        )
                    ...
                    print(
                        "method %s not recognized, please check Fitting.py" % (method)
                    )
                    return
