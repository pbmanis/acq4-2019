...
class STDPAnalyzer(AnalysisModule):
    ...
    designed to analyze Spike-Timing Dependent Plasticity data, but can
    be used to measure any manipulation that affects synaptic responses
    over time.
    """
    ...
    def __init__(self, host):
        ...
        self.fileLoader = STDPFileLoader.STDPFileLoader(
            self.dataManager(), host=self, showFileTree=True
        )
        ...
        self._elements_ = OrderedDict(
            [
                ...
                (
                    ...
                    {
                        "type": "ctrl",
                        "object": self.plotsWidget,
                        "pos": ("above", "Pairing Plots"),
                        "size": (1000, 400),
                    },
                ),
            ]
        )
        self._sizeHint = (1280, 800)
        ...
        for p in [
            self.plots.exptPlot,
            self.plots.tracesPlot,
            self.plots.plasticityPlot,
            self.plots.RMP_plot,
            self.plots.RI_plot,
            self.plots.holdingPlot,
        ]:
        ...
        self.ctrl.pspStartTimeSpin.setOpts(
            suffix="s", siPrefix=True, dec=True, value=0.055, step=0.1
        )
        self.ctrl.spikePeakSpin.setOpts(
            suffix="s", siPrefix=True, dec=True, value=0.065, step=0.1
        )
        ...
        self.averageCtrl = pg.WidgetGroup(
            self.ctrl.traceDisplayGroup
        )  ##TODO: save state when we save data
        self.ctrl.averageCheck.setChecked(True)
        self.ctrl.averageTimeSpin.setOpts(
            suffix="s", siPrefix=True, dec=True, value=60, step=1
        )
        self.ctrl.averageNumberSpin.setOpts(step=1, dec=True)
        self.ctrl.startExcludeAPsSpin.setOpts(
            suffix="s", siPrefix=True, dec=True, value=0, step=1, minStep=0.001
        )
        self.ctrl.endExcludeAPsSpin.setOpts(
            suffix="s", siPrefix=True, dec=True, value=0.25, step=1, minStep=0.001
        )
        ...
        self.ctrl.baselineStartSpin.setOpts(
            suffix="s", siPrefix=True, dec=True, step=1, minStep=0.001
        )
        self.ctrl.baselineEndSpin.setOpts(
            suffix="s", siPrefix=True, dec=True, step=1, minStep=0.001
        )
        ...
        self.ctrl.pspStartSpin.setOpts(
            suffix="s", siPrefix=True, dec=True, step=1, minStep=0.001
        )
        self.ctrl.pspEndSpin.setOpts(
            suffix="s", siPrefix=True, dec=True, step=1, minStep=0.001
        )
        ...
        self.ctrl.healthStartSpin.setOpts(
            suffix="s", siPrefix=True, dec=True, step=1, minStep=0.001
        )
        self.ctrl.healthEndSpin.setOpts(
            suffix="s", siPrefix=True, dec=True, step=1, minStep=0.001
        )
        ...
        self.ctrl.storeToPklBtn.clicked.connect(self.storeToPkl)
        ...
        self.baselineRgn.setRegion((0, 0.0095))
        self.pspRgn.setRegion((0.012, 0.016))
        self.healthRgn.setRegion((0.215, 0.275))
    ...
    def loadEPSPFileRequested(self, files):
        ...
        with pg.ProgressDialog("Loading data..", 0, n) as dlg:
            for f in files:
                arr = np.zeros(
                    (len(f.ls())), dtype=[("timestamp", float), ("data", object)]
                )
                maxi = -1
                ...
                self.traces = np.concatenate(
                    (self.traces, arr[:maxi])
                )  # only concatenate successfully read traces
                self.lastAverageState = {}
                self.files.append(f)
    ...
    def updateExptPlot(self):
        ...
        if self.ctrl.averageCheck.isChecked():
            self.plots.exptPlot.plot(
                x=self.traces["timestamp"] - self.expStart,
                y=[1] * len(self.traces),
                pen=None,
                symbol="o",
                symbolSize=6,
            ...
            self.plots.exptPlot.plot(
                x=self.averagedTraces["avgTimeStamp"] - self.expStart,
                y=[2] * len(self.averagedTraces),
                pen=None,
                symbol="o",
                symbolSize=6,
                symbolBrush=(255, 0, 0),
        ...
        if self.ctrl.excludeAPsCheck.isChecked():
            self.plots.exptPlot.plot(
                x=self.excludedTraces["timestamp"] - self.expStart,
                y=[1] * len(self.excludedTraces),
                pen=None,
                symbol="o",
                symbolSize=6,
                ...
                if displayOrig:
                    for t in data["origTimes"][i]:
                        ...
                        self.plots.tracesPlot.plot(
                            orig["primary"], pen=pg.intColor(i, len(data), alpha=30)
                        )
        ...
        if self.analysisResults is not None and len(data[dataKey]) > 0:
            ...
            for i, time in enumerate(data[timeKey]):
                ...
                self.plots.tracesPlot.plot(
                    ...
                    symbol="o",
                    symbolPen=None,
                    symbolBrush=pg.intColor(i, len(data)),
                )

    def updatePairingPlot(self):
        self.pairingPlot.clear()
        avgTrace = (
            self.pairingTraces[0]["data"]["primary"]
            - self.pairingTraces[0]["data"]["primary"]
        )  ## get an empty array with the same structure as the data
        avgN = 0
    ...
    def resetAveragedTraces(self, n=0):
        ...
        self.averagedTraces = np.zeros(
            n,
            dtype=[("avgTimeStamp", float), 
                   ("avgData", object), 
                   ("origTimes", object)],
        )
    ...
    def averageCtrlChanged(self):
        if (
            not self.ctrl.averageCheck.isChecked()
        ):  ## if we're not averaging anyway, we don't need to do anything
            self.updateExptPlot()
            self.updateTracesPlot()
            return

        if (
            not self.needNewAverage()
        ):  ## if the parameters for averaging didn't change, we don't need to do anything
            self.updateTracesPlot()
            self.updateExptPlot()
            return
    ...
    def needNewAverage(self):
        ...
        if excludeAPs:
            ...
            if not (
                (start == self.lastAverageState.get("startExcludeAPsTime", None))
                and (end == self.lastAverageState.get("endExcludeAPsTime", None))
            ):
                return True
        ...
        if self.ctrl.averageTimeRadio.isChecked():
            ...
            if (method == self.lastAverageState.get("method", None)) and (
                value == self.lastAverageState.get("value", None)
            ):
                return False
            else:
                return True
    ...
    def getNewAverages(self):
        ...
        if self.ctrl.averageTimeRadio.isChecked():
            ...
            raise Exception(
                "Unable to average traces. Please make sure an averaging method is selected."
            )
        ...
        self.lastAverageState = {
            ...
            "startExcludeAPsTime": start,
            "endExcludeAPsTime": end,
        }
    ...
    def excludeAPs(self):
        timeWindow = (
            self.ctrl.startExcludeAPsSpin.value(),
            self.ctrl.endExcludeAPsSpin.value(),
        )
    ...
    def averageByTime(self, time, excludeAPs=False):
        n = int((self.traces["timestamp"].max() - self.expStart) / time) + (
            (self.traces["timestamp"].max() - self.expStart) % time > 0
        )  ### weird solution for rounding up
        ...
        while t < len(includedTraces):
            traces = includedTraces[
                (includedTraces["timestamp"] >= self.expStart + time * i)
                * (includedTraces["timestamp"] < self.expStart + time * i + time)
            ]
            if len(traces) > 1:
                x = traces[0]["data"]
        ...
        self.averagedTraces = self.averagedTraces[
            self.averagedTraces["avgTimeStamp"] != 0
        ]  ## clean up any left over zeros from pauses in data collection

    def averageByNumber(self, number, excludeAPs=False):
        n = int(len(self.traces) / number) + (
            len(self.traces) % number > 0
        )  ### weird solution for rounding up
        self.resetAveragedTraces(n)
    ...
    def measureModeChanged(self):
        self.analysisMode = (
            self.ctrl.measureModeCombo.currentText()
        )  # reall don't use this, just read directly from combo box
    ...
    def analyze(self):
        ...
        self.analysisResults = np.zeros(
            len(traces),
            dtype=[
                ("time", float),  # record array...
                ("RMP", float),
                ("pspSlope", float),
        ...
        self.plots.holdingPlot.plot(
            x=times - self.expStart,
            y=self.analysisResults["holdingCurrent"],
            pen=None,
            symbol="o",
            symbolSize=symsize,
            symbolPen=None,
        ...
        if self.ctrl.healthCheck.isChecked():
            self.plots.RI_plot.plot(
                x=times - self.expStart,
                y=self.analysisResults["inputResistance"],
                pen=None,
                symbol="o",
                symbolSize=symsize,
                symbolPen=None,
        ...
        if self.ctrl.baselineCheck.isChecked():
            self.plots.RMP_plot.plot(
                x=times - self.expStart,
                y=self.analysisResults["RMP"],
                pen=None,
                symbol="o",
                symbolSize=symsize,
                symbolPen=None,
    ...
    def updatePlot(self):
        ...
        postwin = [
            self.ctrl.plasticityRgnStartSpin.value(),
            self.ctrl.plasticityRgnEndSpin.value(),
        ]
        ...
        if self.ctrl.pspCheck.isChecked():
            ...
            if self.ctrl.measureModeCombo.currentText().startswith("Slope"):
                self.plots.plasticityPlot.plot(
                    x=times - self.expStart,
                    y=self.analysisResults["pspSlope"],
                    pen=None,
                    symbol="o",
                    symbolSize=symsize,
                    symbolPen=None,
                ...
                base, basetime = (
                    self.analysisResults["pspSlope"][:basepts].mean(),
                    [
                        self.analysisResults["time"][:basepts][0] - self.expStart,
                        self.analysisResults["time"][:basepts][-1] - self.expStart,
                    ],
                )
                ...
                try:
                    ...
                    msg += "Max rec time: %8.3f sec, window starts at %8.3f sec" % (
                        np.max(self.analysisResults["time"]),
                        60.0 * postwin[0],
                    )
                ...
                post, postTime = (
                    self.analysisResults["pspSlope"][postRgn[0] : postRgn[1]].mean(),
                    [
                        self.analysisResults["time"][postRgn[0]] - self.expStart,
                        self.analysisResults["time"][postRgn[1]] - self.expStart,
                    ],
                )
                ...
                self.plots.plasticityPlot.plot(
                    x=times - self.expStart,
                    y=self.analysisResults["pspAmplitude"],
                    pen=None,
                    symbol="o",
                    symbolSize=symsize,
                    symbolPen=None,
                ...
                self.plots.plasticityPlot.plot(
                    x=times - self.expStart,
                    y=self.analysisResults["pspArea"],
                    pen=None,
                    symbol="o",
                    symbolSize=symsize,
                    symbolPen=None,
        ...
        for i, trace in enumerate(traces):
            ...
            while t < len(data["primary"]):
                ...
                if s[0] > slope:
                    slope = s[0]
                    offset = s[1]
                    hightime = rgn2[0] + t * timestep - (region * timestep) / 2.0
                t += step
            ...
            if self.analysisResults["RMP"][i] != 0:
                ...
                baseline = data["primary"][
                    :5
                ].mean()  ## if we don't have a baseline measurement, just use the first 5 points
            peakPosition = np.argwhere(data["primary"] == data["primary"].max())[0][0]
            avgPeak = (
                data["primary"][
                    peakPosition - int(5 / 2) : peakPosition + int(5 / 2)
                ].mean()
                - baseline
            )
    ...
    def measureHealth(self, traces):
        ...
        for i, trace in enumerate(traces):
            ...
            self.analysisResults[i]["bridgeBalance"] = (
                bridgeCompensation + R["bridgeBalance"]
            )

    def storeToPkl(self):  
        """
        Send to a pickled dictionary file
        """
        # build dict for this entry:
        data = dict()
        data["CellDir"] = self.dataModel.getParent(self.files[0], "Cell").name()
        data["includedProtocols"] = [Path(n.name()).name for n in self.files] 
        ...
        baselineSlope = self.analysisResults[
            self.analysisResults["time"] < (self.expStart + 300)
            ]["pspSlope"].mean()
        data["timestamp"] = self.analysisResults["time"]
        data["time"] = self.analysisResults["time"] - self.expStart
        data["RMP"] = self.analysisResults["RMP"]
        data["inputResistance"] = self.analysisResults["inputResistance"]
        data["holdingCurrent"] = self.analysisResults['holdingCurrent']
        data["bridgeBalance"] = self.analysisResults["bridgeBalance"]
        data["tau"] = self.analysisResults["tau"]
        data["pspSlope"] = self.analysisResults["pspSlope"]
        data["normalizedPspSlope"] = self.analysisResults["pspSlope"] / baselineSlope
        data["slopeFitOffset"] = self.analysisResults["slopeFitOffset"]
        data["highSlopeLocation"] = self.analysisResults["highSlopeLocation"]
        data["pspAmplitude"] = self.analysisResults["pspAmplitude"]
        data["pspArea"] = self.analysisResults["pspArea"]
    ...
    def storeTrialsToDB(self):
        ...
        db.checkTable(
            table,
            owner=self.dbIdentity + ".trials",
            columns=trialFields,
            create=True,
            addUnknownColumns=True,
            indexes=[["CellDir"], ["ProtocolSequenceDir"]],
        )

        data = np.zeros(
            len(self.analysisResults),
            dtype=[
                ("CellDir", object),
        ...
        for i in range(len(self.analysisResults)):
            ...
            data[i]["normalizedPspSlope"] = (
                self.analysisResults[i]["pspSlope"] / baselineSlope
            )
            ...
            data[i]["includedProtocols"] = self.averagedTraces[i][
                "origTimes"
            ]  ### TODO: make this protocolDirs instead of timestamps....

        old = db.select(table, where={"CellDir": data["CellDir"][0]}, toArray=True)
        if (
            old is not None
        ):  ## only do deleting if there is already data stored for this cell
    ...
    def createSummarySheet(self, showPlasticity=True):
        ...
        plot.addLegend()
        plot.plot(
            x=timeValues[int(0.045 / rate) : int(0.085 / rate)],
            y=baseAvg[int(0.045 / rate) : int(0.085 / rate)],
            pen="b",
            name="Baseline",
        )
        if showPlasticity:
            plot.plot(
                x=timeValues[int(0.045 / rate) : int(0.085 / rate)],
                y=postAvg[int(0.045 / rate) : int(0.085 / rate)],
                pen="r",
                name="Post-pairing",
            )
        ...
        if showPlasticity:
            plasticity = "%g %%" % (self.plasticity * 100.0)
        else:
        ...
        info = (
            "Antagonist: %s <br /> Agonist: %s <br /><br />Pairing Interval: %f<br /><br /> Plasticity: %s"
            % (antagonist, agonist, interval, plasticity)
        )
        ...
        pairingPlot.plot(
            timeValues[int(0.05 / rate) : int(0.15 / rate)],
            self.avgPairingTrace["Time":0.05:0.15],
        )
        l.addItem(pairingPlot, row=1, col=1, rowspan=2)

        plasticityPlot = pg.PlotItem(
            x=self.analysisResults["time"] - self.expStart,
            y=self.analysisResults["pspSlope"],
            pen=None,
            symbol="o",
            symbolSize=5,
