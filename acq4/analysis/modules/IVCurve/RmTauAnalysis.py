...
class RmTauAnalysis:
    ...
    def tau_membrane(
        ...
        rgnpk = sorted(rgnpk)
        vrange = np.sort(vrange) / 1000.0
        rgnindx = [int((rgnpk[1] - 0.005) / dt), int((rgnpk[1]) / dt)]
        rmps = self.ivbaseline
        vmeans = (
            np.mean(self.Clamps.traces[:, rgnindx[0] : rgnindx[1]], axis=1)
            - self.ivbaseline
        )
        # vrange is in mV
        indxs = np.where(
        ...
        for j, k in enumerate(whichdata):
            if self.dataPlot is not None:
                n = "w_%03d" % j
                self.taum_fitted[n] = self.dataPlot.plot(
                    self.Clamps.time_base, self.Clamps.traces[k], pen=pg.mkPen("w")
                )
    ...
    def rmp_analysis(self, region=None):
        ...
        if region is None:
            raise ValueError(
                "IVCurve, rmp_analysis requires a region beginning and end to measure the RMP"
            )
    ...
    def tau_h(self, current, rgn, pkRgn, ssRgn, printWindow=False):
        ...
        vrmp = np.median(target["Time" : 0.0 : self.Clamps.tstart - 0.005]) * 1000.0
        self.tauh_vrmp = vrmp
        # get peak and steady-state voltages
        vpk = target["Time" : pkRgn[0] : pkRgn[1]].min() * 1000
        self.tauh_vpk = vpk
        self.tauh_neg_pk = (vpk - vrmp) / 1000.0
        vss = np.median(target["Time" : ssRgn[0] : ssRgn[1]]) * 1000
        self.tauh_vss = vss
        self.tauh_neg_ss = (vss - vrmp) / 1000.0
        whichdata = [int(amin)]
