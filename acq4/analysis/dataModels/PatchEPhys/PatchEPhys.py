...
ic_modes = ['IC', 'IC', 'CC', 'IClamp', 'ic', 'I-Clamp Fast', 'I-Clamp Slow']
...
def getClampCommand(data, generateEmpty=True):
    ...
    if data.hasColumn('Channel', b"'Command'"):  # why?
        return data['Channel': 'Command']
    elif data.hasColumn('Channel', b"'command'"):
        return data['Channel': 'command']
    else:
        if generateEmpty:
    ...
    return None  # you can't get here.... 

def getClampPrimary(data):
    """Return primary channel from
        the data
        """
    # print('has column primary: ', data.hasColumn('Channel', b"'primary'"))
    if data.hasColumn('Channel', b"'primary'"):
        return data['Channel': 'primary']
    elif data.hasColumn('Channel', b"'scaled'"):
        return data['Channel': 'scaled']
    else:
        print('cannot find channel in data: ', data)
        raise ValueError
...
def getClampMode(data_handle, dir_handle=None):
    ...
    if 'ClampState' in info:
        return info['ClampState']['mode'].decode('utf-8')[1:-1]  # remove single quotes from 
    else:

        try:
            mode = info['mode'].decode('utf-8')[1:-1] # if the mode is in the info (sometimes), return that
            return mode
        except KeyError:
